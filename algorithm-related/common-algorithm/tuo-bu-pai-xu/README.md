...
在图论中，由一个有向无环图的顶点组成的序列，当且仅当满足下列条件时，才能称为该图的一个**拓扑排序**（英语：Topological sorting）：
